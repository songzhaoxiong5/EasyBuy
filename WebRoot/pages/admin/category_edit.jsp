<%@ page language="java" import="java.util.*,com.szx.service.impl.*,com.szx.bean.*,com.szx.service.*" pageEncoding="UTF-8"%>
...
pageContext.setAttribute("id", categoryId);
ProductCategoryService categoryService=new ProductCategoryServiceImpl();
EasyBuy_ProductCategory category=categoryService.selectOne("id",categoryId);
...
<div class="container-fluid">
 ...
 <div class="row-fluid">
	...
	<div class="btn-toolbar">
		    ...
		    <a href="#myModal" data-toggle="modal" class="btn" id="upperDelButton" rel="${pageScope.category.id}">Delete</a>
...
<div class="well">
    <form id="categoryModifyForm" method="get" action="">
    	<input type="hidden" name="opr" value="modify">
        <label>Category Name</label>
        <input type="text" value="${pageScope.category.name}" class="input-xlarge" name="name">
        <label>Category Type</label>
       	<label class="radio">
  		<input type="radio" name="type"  value="1" <c:if test="${pageScope.category.type == 1}"><c:out value="checked"/></c:if> />First
		</label>
		<label class="radio">
  		<input type="radio" name="type"  value="2" <c:if test="${pageScope.category.type == 2}"><c:out value="checked"/></c:if> />Second
		</label>
		<label class="radio">
  		<input type="radio" name="type"  value="3" <c:if test="${pageScope.category.type == 3}"><c:out value="checked"/></c:if> />Third
		</label>
		<input type="hidden" name="parentId" id="parentId" value="${pageScope.category.parentId}">
		<input type="hidden" name="id" value="${pageScope.category.id}">
		...
		<div class="btn-group">
			  ...
			  <ul class="dropdown-menu" id="select">
			    <!-- dropdown menu links -->
			    	<li ><a href="javascript:;" rel="0">Please Select！</a></li>
			  </ul>
		</div>
		<br/>
    ...
    <script type="text/javascript">
   	$(document).ready(function(){
	    	...
	    	var $formArea=$("#categoryModifyForm");//获取表单
	    	var $inputs=$formArea.find(":input");//所有的表单input元素
	    	var $select=$("#select");//获取父类按钮下拉菜单
	    	$("div.span9").on("click","#saveBtn",function(){
	    		if($("#categoryModifyForm").valid()){
	    	...
	    	function initParentName(){
	    		if($("#parentId").val()!=undefined&&$("#parentId").val()!=null&&$("#parentId").val()!=""){
	    			$.getJSON("categorycontrol","opr=listAllWithClues&clueNumber=1&clue1=id&clueValue1="+$("#parentId").val(),fillParent);
	    			function fillParent(data){
	    				if(data[1][0]!=null&&data[1][0]!=undefined){
	    					var $parentName=data[1][0].name;
		    	...
		    	$("#modalDel").click(function(){
		    		...
		    		var data="opr=del&delId="+$("#delId").val();
			    		$.getJSON("categorycontrol",data,afterManipulation);
		    	});
