...
pageContext.setAttribute("id", productId);
ProductServiceImpl productService=new ProductServiceImpl();
EasyBuy_Product product=productService.selectOne("id",productId);
pageContext.setAttribute("product", product);
String relativePath="#";
String absolutePath=product.getImg();
if(absolutePath!=null){
	relativePath=absolutePath.substring(absolutePath.indexOf("EasyBuy")+8);
}
%>
   ...
   <%@ include file="console_elements/sidebar.jsp" %>
		...
		<div class="well">		      
			    <div class="box-content">
							<form class="form-horizontal" id="productModifyForm" method="post" action="" enctype="multipart/form-data">
							  <fieldset>
							  <div id="result" style="display:none;" class="alert alert-info" role="alert"></div> 
								<legend id="legend">Modify Product</legend>
								<input type="hidden" name="id" value="${pageScope.product.id}">
							 	<input type="hidden" name="opr" value="modifyWithFile">
								<div class="control-group">
								  <label class="control-label">Name</label>
								  <div class="controls">
									<input type="text" class="span6"  name="name" value="${pageScope.product.name}">
								  </div>
								</div>
								<div class="control-group">
								  <label class="control-label">Description</label>
								  <div class="controls">
									<textarea  id="textarea" rows="5"  name="description" value="" class="span6">${pageScope.product.description}</textarea>
								  </div>
								</div>
								<div class="control-group">
								  <label class="control-label">Price</label>
								  <div class="controls">
									<input type="text" class="span6"  name="price" value="${pageScope.product.price}">
								  </div>
								</div>
								<div class="control-group">
								  <label class="control-label">Stock</label>
								  <div class="controls">
									<input type="text" class="span6"  name="stock" value="${pageScope.product.stock}">
								  </div>
								</div>
								<div class="control-group">
								  ...
								  <div class="controls">
									<select name="categoryLevel1" id="select1">
										...
										<option value="${pageScope.product.categoryLevel1}" selected id="prev1"></option>
									</select>
								  </div>
								</div>
								<div class="control-group">
								  ...
								  <div class="controls">
									<select name="categoryLevel2" id="select2">
										...
										<option value="${pageScope.product.categoryLevel2}" selected id="prev2"></option>
									</select>
								  </div>
								</div>
								<div class="control-group">
								  ...
								  <div class="controls">
									<select name="categoryLevel3" id="select3">
										...
										<option value="${pageScope.product.categoryLevel3}" selected id="prev3"></option>
									</select>
								  </div>
								</div>
								...
								<div class="control-group">
								  ...
								  <div class="controls">
										<label class="radio">
								  		<input type="radio" name="isDelete"  value="0"  <c:if test="${pageScope.product.isDelete == 0}"><c:out value="checked"/></c:if> > On Shelf
										</label>
										<label class="radio">
								  		<input type="radio" name="isDelete"  value="1" <c:if test="${pageScope.product.isDelete == 1}"><c:out value="checked"/></c:if> > Off Shelf
										</label>
								  </div>
								</div>
								<img src="<%=relativePath%>" class='img-polaroid' align="center" >
								<input type="hidden" value="${pageScope.product.img}" name="img">
								<div class="control-group">
	...
	<script src="${pageScope.basePath}js/self/admin/dynamic-select2.js" type="text/javascript"></script>
...
$(document).ready(function(){
		...
		$("#submitButton").click(function(){
				if($("#productModifyForm").valid()){
				var myForm=document.getElementById("productModifyForm");//获取前台表单
    	...
    	$("#modalDel").click(function(){
    		...
    		var data="opr=del&delId="+$("#delId").val();
	    		$.getJSON("productcontrol",data,afterManipulation);
    	});
    	$("div.span9").on("click","#goBackToList",function(){
				location.href="pages/admin/products.jsp";
		});
